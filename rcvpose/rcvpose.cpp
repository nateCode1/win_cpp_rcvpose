...
RCVpose::RCVpose(Options& options)
{
    ...
    if (!opts.resume_train) {
        ...
        std::cout<<"NOT RESUMING TRAINING"<<std::endl;
        try {
            ...
            auto device_ids = torch::arange(num_devices, torch::kCUDA);

            model = DenseFCNResNet152(3, 4);
            model->to(device_type);
            
            if ((device_type == torch::kCUDA) && (opts.verbose)) {
        	...
        	std::cout << "GPU " << i << " memory:" << std::endl;
        ...
        try {
        std::cout<<"RESUMING TRAINING/LOADING BEST MODEL"<<std::endl;
            //model = DenseFCNResNet152(3, 4);
